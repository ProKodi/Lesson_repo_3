...
Разработать приложение для работы с БД:
    ...
    Третья форма – обработка запроса. Выдать данные о студентах,  фамилия  которых 
    начинается на букву П.  
    Четвертая форма – обработка запроса. Рассчитать средний балл по всем студентам по каждому студенту .

1) Разработать  главную  экранную  форму  с  четырьмя  кнопками:  
    ...
    Четвертая  - выход  из программы.  
        Для  каждой  кнопки  прописать  соответствующий  код  либо открытие новой формы, 
            либо – закрытие главной формы.

2) На форму добавить таблицу в которую потом будем данные писать

3) Тоже самое что 2 но с возможностью ввода 
